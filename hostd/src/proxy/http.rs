...
//! bearer JWT, re-validate the target against live VM state (shared with
//! the TCP mode — see `target.rs`), and forward the request to
//! `http://<guest_ip>:<port>`.
...
use super::target::{ProxyError, resolve_target};
